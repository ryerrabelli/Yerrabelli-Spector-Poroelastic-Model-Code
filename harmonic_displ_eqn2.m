...
function [ displ ] = harmonic_displ_eqn2( times, v, E, r, a, alph, omega, tg, shouldDimensionalize)
...
%   Detailed explanation goes here
    ...
    %below is the equation for displacement divided by a
    displ = -v31*rp + real( (A*besseli(1,rtia*rp)-rp/2).*exp(i*omega*times) );
    if (shouldDimensionalize) %if the result should be dimensionalized, then multiply by a
        displ = displ * a;
    end
    %}
    ...
    fnc= A*besseli(1,rtia*rp)-rp/2; %divided by eps0
    displ= -v31*rp + abs(fnc) * cos(omega*times + angle(fnc) );
